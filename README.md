...
- **Offline write queue (expenses)** — logging or editing a plain (non-recurring) expense, or deleting one with no scope, works offline and syncs automatically once back online; the write is queued already-encrypted (the ledger key never touches disk) and replays safely if the connection drops mid-send. Editing an already-recurring expense, deleting one with a scope, and every other entity type still require a connection
